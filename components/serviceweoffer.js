  ...
  return (
      ...
      <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4 lg:gap-6 mt-4">
        {servicedata.map((el, idx) => {
          return (
            <div className="shadow-xl rounded-xl text-center mt-6 border border-slate-300 py-4"
            key={idx}>
      ...
      <button onClick={onclick} className="font-semibold text-[20px] rounded-lg bg-black text-white px-8 py-3 mt-8 hover:bg-[#e8f1e8] hover:text-black">Get in Touch</button>
      </div>
      </div>
    </div>
  )
